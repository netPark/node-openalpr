...
NAN_METHOD (Start)
{
	...
	int instancesCount = Nan::To<int32_t>(info[2]).FromMaybe(0);
...
NAN_METHOD (IdentifyLicense)
{
	...
	bool detectRegion = Nan::To<bool>(info[3]).FromMaybe(false);
	...
	for (int i = 0; i < regionsArray->Length (); i++) {
		...
		int x =       Nan::To<uint32_t>(regionValues->Get (0)).FromMaybe(0);
		int y =       Nan::To<uint32_t>(regionValues->Get (1)).FromMaybe(0);
		int width =   Nan::To<uint32_t>(regionValues->Get (2)).FromMaybe(0);
		int height =  Nan::To<uint32_t>(regionValues->Get (3)).FromMaybe(0);
